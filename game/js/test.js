...
let saveHighBtn = document.querySelector('#saveHighBtn');
let yourNickname = document.querySelector('#yourNickname');
...
let POP_SOUND = new Howl({ src: ['audio/pop.mp3'], rate: 3, volume: 0.1 });
let BULLET_SOUND = new Howl({ src: ['audio/pong.mp3'], rate: 1.5, volume: 0.1 });
let THEME_SONG = new Howl({ src: ['audio/background.mp3'], rate: 1, volume: currentVolume, loop: true });
// general data ----------------------------------------------------------------
    ...
    for (let i = 0; i < targets.length; i++) { // check for triangle-circle (player-target) collision
      ...
      if (tri.collides(targets[i])) {
          ...
          if (C_HEALTH < 100){
            ...
            THEME_SONG.fade(currentVolume, 0, 2000);
...
let gameINIT = function() {
  THEME_SONG.play();
  THEME_SONG.fade(0, currentVolume, 2000);
  isStart = true;
  currObj = canvas;
  bullets = [];
  targets = [];
